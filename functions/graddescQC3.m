...
[Vold,~,E,dV] = qc3_eig_v3(datagen,Slocal,datallo);
Vold = Vold - E;
Dold = datallo;
[m,n] = size(Dold);
...
if showProgress && track && n==2
    ...
    [Vmesh,~,~,dVgrid] = qc3_eig_v3(datagen(:,1:size(datagrid2,2)),Slocal,datagrid2);
...
while (t <= steps) && (( convergence >= ERR) || ( convergenceV >= ERR))
    ...
    [V,~,E,dV] = qc3_eig_v3(datagen,Slocal,Dnew);
    Vnew = V - E;
